"""Extract SAE activations for 3 example stocks and save for visualization.
Produces a JSON with per-window top-k feature activations and their concept labels.
"""
...
OUTPUT = "/data/houwanlong/finllm-mi/outputs/sae/activation_viz.json"

LAYER, WINDOW, STRIDE = 6, 64, 32
EXPANSION, K, STEPS, BATCH_SIZE, LR = 4, 64, 3000, 256, 1e-4
STOCKS = ["sh600036.csv", "sh600028.csv", "sh600079.csv"]  # 3 diverse stocks
...
for fname in STOCKS:
    ...
    # Get latent activations for ALL windows
    with torch.no_grad():
    ...
    # Get top-20 most active features and their concept labels
    freq = (lat != 0).sum(axis=0)
    top20 = np.argsort(freq)[-20:][::-1]
    feature_labels = {}
    for j in top20:
        a = lat[:,j] != 0
        if a.sum() < 5:
            feature_labels[int(j)] = "unknown"
            continue
        corrs = []
        for k in range(len(LABEL_NAMES)):
            if k < labels_arr.shape[1]:
                cc = np.corrcoef(lat[a,j], labels_arr[a,k])[0,1]
                corrs.append(0 if np.isnan(cc) else abs(cc))
            else:
                corrs.append(0)
        best = np.argmax(corrs)
        feature_labels[int(j)] = LABEL_NAMES[best] if corrs[best] > 0.15 else "unlabeled"

    # Build activation matrix: windows x top-20 features
    act_matrix = lat[:, top20].tolist()
    close_prices = close_raw[:len(wins)*STRIDE:STRIDE][:len(wins)].tolist()

    results[fname.replace(".csv","")] = {
        "n_windows": len(wins),
        "top20_features": [int(j) for j in top20],
        "feature_labels": {str(j): feature_labels[int(j)] for j in top20},
        "activation_matrix": act_matrix[:min(80, len(act_matrix))],  # first 80 windows
        "close_prices": close_prices[:min(80, len(close_prices))],
    }
    del sae; torch.cuda.empty_cache()
    print(f"  Done: {len(wins)} windows, top-20 features labeled")
