...
for fi, fname in enumerate(csvs):
    ...
    # Activations
    acts = []
    def hook_fn(m,i,o): acts.append((o[0] if isinstance(o,tuple) else o)[:,-1,:].detach().cpu().float().numpy())
    ...
    for i in range(0, len(dn)-84, 32):
        ...
        feats_list.append([c[-1]/c[-6]-1 if len(c)>=6 else 0, np.polyfit(np.arange(64),c,1)[0],
            ...
            np.max(r), np.max(np.abs(r)), np.mean(r**2)/(np.var(r)+1e-10)])
    labels = np.array(feats_list)[:len(acts)]
    n_feats = labels.shape[1]

    # SAE features (simple TopK)
class SAE(torch.nn.Module):
        def __init__(self):
            super().__init__()
            self.enc = torch.nn.Linear(d_model, d_model*2, bias=True)
            self.dec = torch.nn.Linear(d_model*2, d_model, bias=False)
            self.b = torch.nn.Parameter(torch.zeros(d_model))
        def encode(self, x):
            xc = x - self.b
            lat = self.enc(xc)
            _, idx = torch.topk(lat, 64, dim=-1)
    ...
    for s in range(3000):
        ...
        loss = torch.nn.functional.mse_loss(sae.dec(xr), at[idx])
        opt.zero_grad(); loss.backward()
        torch.nn.utils.clip_grad_norm_(sae.parameters(), 1.0); opt.step()

    with torch.no_grad(): sae_feats = sae.encode(at[:len(acts)]).cpu().numpy()

    # PCA basis
    pca = PCA(n_components=min(64, d_model))
    pca_feats = pca.fit_transform(acts)

    # Random basis
    np.random.seed(42)
    rand_basis = np.random.randn(d_model, 64)
    rand_basis /= np.linalg.norm(rand_basis, axis=0)
    rand_feats = acts @ rand_basis

    for name, feats in [("sae", sae_feats), ("pca", pca_feats), ("random", rand_feats)]:
        # Get max |corr| for each feature
        max_corrs = []
        for j in range(feats.shape[1]):
            a = feats[:,j] != 0 if name == "sae" else np.ones(len(feats), dtype=bool)
            if a.sum() < 5: continue
        ...
        # Null: shuffle labels 100 times
        null_maxes = []
        shuf = labels.copy()
        for _ in range(50):
            for c in range(n_feats): np.random.shuffle(shuf[:,c])
            nm = []
            for j in range(feats.shape[1]):
                a = feats[:,j] != 0 if name == "sae" else np.ones(len(feats), dtype=bool)
                if a.sum() < 5: continue
            ...
            null_maxes.append(max(nm) if nm else 0)

        null_95 = np.percentile(null_maxes, 95)
    ...
    if (fi+1) % 5 == 0: print(f"[{fi+1}/{len(csvs)}]")
